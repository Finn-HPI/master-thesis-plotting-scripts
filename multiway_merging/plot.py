...
file_paths = [
    ...
    "nx05_merge_bench.csv"
]

def main():
    # parser = argparse.ArgumentParser(
    #     description="Load Hyrise benchmark CSV output into a DataFrame."
    # )
    # parser.add_argument("filepath", help="Path to the benchmark output file")
    ...
    # df["Instr./Tup."] = df["inst_retired"] / df["Tuples"]
    # df["Retiring"] = (
    #     100
    #     * (df["op_retired"] / df["op_spec"])
    #     * (1 - (df["stall_slot"] / (df["cpu_cycles"] * 8)))
    # )
    # df["Stall Backend"] = (df["stall_backend"] / df["cpu_cycles"]) * 100
    # df['L2-Miss/Tup.'] = df['l2d_cache_refill'] / df['Tuples']
    # df['L3-Miss/Tup.'] = df['l3d_cache_refill'] / df['Tuples']

    # print(unified_df['Algo'])
    ...
    def annotate(data, **kws):
        ...
        # ax.text(.6, .8, r"$\bf{L3\text{-}Cache}$ per Thread" + f" = {cache_sizes_in_mib[1]:.2f} MiB", transform=ax.transAxes, color='#377EB8')#,bbox=dict(facecolor='none', edgecolor='black', boxstyle='round,pad=0'))
        ax.text(
            x, .89,
